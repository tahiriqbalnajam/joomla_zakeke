...
<td>
	...
	<?php if ($item->virtuemart_order_item_id > 0 ) { ?>
		...
		<?php
		// Load Zakeke Helper
		$zakekeAuthHeader = null;
		$zakekeHelperPath = JPATH_ADMINISTRATOR . '/components/com_zakeke/helpers/zakeke.php';
		if (file_exists($zakekeHelperPath)) {
			JLoader::register('ZakekeHelper', $zakekeHelperPath);
			if (class_exists('ZakekeHelper')) {
				$zakekeAuthHeader = ZakekeHelper::getAuthHeader();
			}
		}

		if (!$zakekeAuthHeader) {
			// Optionally log an error or display a message if credentials are not found/configured
			// For now, it will proceed without a valid auth header for Zakeke if not configured
			// Consider adding a fallback or a more robust error message for the admin
			// echo '<div class="uk-alert uk-alert-warning">' . JText::_('COM_ZAKEKE_ERROR_AUTH_HEADER_MISSING') . '</div>';
		}
		...
		//function getToken() {
			...
			if ($zakekeAuthHeader) {
				$headers[] = 'Authorization: ' . $zakekeAuthHeader;
			} else {
				// Fallback to hardcoded or handle error - current code has hardcoded value below
				// For safety, if helper exists but returns null (misconfiguration), avoid using potentially old hardcoded values.
				// If you want to keep the hardcoded as a last resort, uncomment the next line.
				// $headers[] = 'Authorization:  Basic ' . base64_encode("289728:sE8pmx_kAN0gCVNKzZM394W5chVkxiVCSXThn7729RA.");
				// It's better to ensure configuration is correct.
			}
			...
			foreach ($custom_fields as $field) {
				...
				if (count($parts) == 2) {
					...
					if ($key === 'zakek_designid' && !empty($value)) {
						...
						// Set CURL options
						curl_setopt_array($curl, array(
						CURLOPT_URL => 'https://api.zakeke.com/v1/designs/' . $value . '/outputfiles/zip',
						CURLOPT_RETURNTRANSFER => true,
						CURLOPT_ENCODING => '',
						CURLOPT_MAXREDIRS => 10,
						CURLOPT_TIMEOUT => 0,
						...
						// Check for errors
						if (curl_errno($curl)) {
							echo 'Zakeke API Error: ' . curl_error($curl);
							curl_close($curl);
							return false;
						}
						...
						//print_r($data);
						// Check if tempPreviewImageUrl exists
						if (isset($data['url'])) {
							echo '<div class="zakeke-link">';
							echo '<a href="' . htmlspecialchars($data['url']) . '" target="_blank">';
							echo 'Download Zakeke Design Files';
							echo '</a>';
							echo '</div>';
						}else {
