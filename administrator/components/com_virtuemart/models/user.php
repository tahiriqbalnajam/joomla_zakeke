...
//quorvia end
	...
	public function register($user, $new)
	{
		...
		$mailToAdmin = $params->get('mail_to_admin');
		...
		elseif (!$new or (empty($useractivation) and empty($sendpassword) and empty($mailToAdmin)))
		{
			return true;
		}
		...
		if (($params->get('useractivation') < 2) && ($mailToAdmin == 1))
		{
			$emailSubject = JText::sprintf(
				'COM_USERS_EMAIL_ACCOUNT_DETAILS',
				$data['name'],
