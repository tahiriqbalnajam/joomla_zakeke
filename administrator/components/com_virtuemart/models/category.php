...
* @version $Id: category.php 10999 2024-04-17 21:01:32Z  $
*/
...
class VirtueMartModelCategory extends VmModel {
	 ...
	 */
	public function getCategory($virtuemart_category_id=null, $childs=TRUE, $fe = true){

		//if(!empty($virtuemart_category_id)) $this->_id = (int)$virtuemart_category_id;
		if(isset($virtuemart_category_id)){
			$this->_id = $this->setId($virtuemart_category_id);
		...
		if (isset($this->_cache[$this->_id][$childs][VmLanguage::$currLangTag])) {
			...
			$this->_cache[$this->_id][$childs][VmLanguage::$currLangTag]->productcount = 0;
			...
			if(!isset($this->_cache[$this->_id][$childs][VmLanguage::$currLangTag]->productcount)) $this->_cache[$this->_id][$childs][VmLanguage::$currLangTag]->productcount = $this->countProducts($this->_id); */
...
//for( $i=0; $i<count($allCats); $i++ ){
	...
	public function store(&$data) {
		...
		vDispatcher::trigger('plgVmBeforeStoreCategory',array(&$data, &$table));
		...
		if($table->has_medias!=$file_id){
			$q = 'UPDATE #__virtuemart_categories SET `has_medias`='.$file_id.'
	...
	WHERE  `virtuemart_category_id`='.$data['category_parent_id'].';';
		...
		vDispatcher::trigger('plgVmAfterStoreCategory',array(&$data, &$table));
	...
	public function remove($cids) {
		...
		foreach($cids as &$cid) {
			...
			if($table->category_parent_id){
				...
				$q = 'UPDATE #__virtuemart_categories SET `has_children`='.(int)$has_children.'
	WHERE  `virtuemart_category_id`='.$table->category_parent_id.';';
			...
			vDispatcher::importVMPlugins ('vmcustom');
			vDispatcher::trigger ('plgVmOnDeleteCategory', array($cid));
	...
	public static function getParents($virtuemart_category_id, $menuCatid = 0){
		...
		foreach ($parents_id as $id ) {
			if(isset(self::$parentsList[VmLanguage::$currLangTag][$id])){
				$parents[] = self::$parentsList[VmLanguage::$currLangTag][$id];
				vmdebug('getParentsList found cached',$id);
			} else {
				$db->setQuery($q.(int)$id);

				try {
					self::$parentsList[VmLanguage::$currLangTag][$id]=$db->loadObject();
					$parents[] = self::$parentsList[VmLanguage::$currLangTag][$id];
				} catch (Exception $e){
