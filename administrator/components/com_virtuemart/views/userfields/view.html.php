...
class VirtuemartViewUserfields extends VmViewAdmin {
	...
	function display($tpl = null) {
		...
		if ($layoutName == 'edit') {
			...
			$this->userField = $model->getUserfield(vRequest::getInt('virtuemart_userfield_id',0));
			...
			if (($n = count($userFieldValues)) < 1) {
				...
				for ($i = 0; $i < $n; $i++) {
					...
					$lists['userfield_values'] .=
						 '<tr>'
						 .'<td><input type="hidden" value="'.$userFieldValues[$i]->virtuemart_userfield_value_id.'" name="virtuemart_userfield_value_id['.$i.']"><input type="text" value="'.$userFieldValues[$i]->fieldvalue.'" name="vValues['.$i.']" /></td>'
