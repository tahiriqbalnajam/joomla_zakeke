 ...
 * @version $Id: view.html.php 10950 2023-12-19 19:23:06Z  $
 */
...
class VirtuemartViewUpdatesMigration extends VmViewAdmin {
	...
	public function renderTaskButton($task, $descr, $extra='', $type ='a'){

		$link= JRoute::_('index.php?option=com_virtuemart&view=updatesmigration&task='.$task.'&'.JSession::getFormToken().'=1'.$extra );
		$html = '<div class="icon">';
		if($type=='a'){
			//$html .= '<'.$type.' onclick="if(!confirm(\''.addslashes( vmText::_($descr.'_CONFIRM_JS') ).'\', \''.$link.'\'));">';
			$html .= '<'.$type.' class="vmjs-confirm" data-url="'.$link.'" data-query="'.vmText::_($descr.'_CONFIRM_JS').'" >';
		} else {
			$html .= '<'.$type.' class="vmjs-confirm" data-task="'.$task.'" data-query="'.vmText::_($descr.'_CONFIRM_JS').'" >';
		}


		$html .= '<span class="vmicon48"></span>';
		$html .= '<br />'.vmText::_($descr).'</'.$type.'></div>';
		return $html;
	}
