...
foreach($this->userFields['fields'] as $field) {
	...
	if($field['type'] == 'delimiter') {
		...
		if($field['name']=='delimiter_userinfo'){
			if($this->getBaseLayout() == 'edit'){
				echo $this->loadTemplate('vmshopper');
			}

		} else {
			?>
			<fieldset<?php /* DWP */ echo(!empty($field['html_classes']) ? ' class="'.$field['html_classes'].'"' : '') ?>>
			<legend class="userfields_info<?php /* DWP */ echo(!empty($field['html_classes']) ? ' '.$field['html_classes'] : '') ?>"><?php echo $field['title'] ?></legend>
		...
		// Output: Userfields
		?>
				<tr title="<?php echo strip_tags($descr) ?>"<?php /* DWP */ echo(!empty($field['html_classes']) ? ' class="'.$field['html_classes'].'"' : '') ?>>
					<td class="key"  >
