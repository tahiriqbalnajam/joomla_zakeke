...
<td>
	...
	<?php if ($item->virtuemart_order_item_id > 0 ) { ?>
		<div class="goto-product">
			...
			<?php
			if (!empty($item->product_customfields)) {
            foreach ($item->product_customfields as $customfield) {
                if ($customfield->custom_title == 'zakek_designid' && !empty($customfield->customfield_value)) {
                    echo '<div class="zakeke-designid">';
                    echo '<strong>Zakeke Design ID:</strong> ' . htmlspecialchars($customfield->customfield_value);
                    echo '</div>';
                }
            }
        } ?>
...
$query = $db->getQuery(true)
    ->select('*')
    ->from('#__virtuemart_order_item_customfields')
    ->where('virtuemart_order_item_id = ' . $item->virtuemart_order_item_id);
$db->setQuery($query);
...
if (!empty($customFields)) {
    foreach ($customFields as $field) {
        if ($field->custom_title === 'zakek_designid') {
            ...
            echo '<a href="https://api.zakeke.com/preview/' . $field->customfield_value . '" target="_blank">';
            echo 'View Design #' . htmlspecialchars($field->customfield_value);
            echo '</a>';
            echo '</div>';
        } else {
            // Display other custom fields normally
            echo '<div class="custom-field">';
            echo '<span class="custom-title">' . htmlspecialchars($field->custom_title) . ':</span> ';
            echo htmlspecialchars($field->customfield_value);
            echo '</div>';
        }
    }
}
