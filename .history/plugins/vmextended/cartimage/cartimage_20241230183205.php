...
class PlgVMExtendedCartimage extends vmCustomPlugin
{
    ...
    public function plgVmConfirmedOrder($cart, $order)
    {
        ...
        if (!empty($designId)) {
            ...
            // Set cURL options
            curl_setopt_array($curl, [
                CURLOPT_URL => 'https://api.zakeke.com/v2/order/' . $orderId, // Replace with your API endpoint
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 30, // Timeout set to 30 seconds
                CURLOPT_FOLLOWLOCATION => true,
                CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                CURLOPT_CUSTOMREQUEST => 'GET', // Use HTTP GET method
                CURLOPT_HTTPHEADER => [
                    'Authorization: Bearer ' . $token, // Bearer token for authentication
            ...
            if (curl_errno($curl)) {
                ...
                print_r($decodedResponse); // Print the order details
            }
