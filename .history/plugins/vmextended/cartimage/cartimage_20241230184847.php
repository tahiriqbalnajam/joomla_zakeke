...
class PlgVMExtendedCartimage extends vmCustomPlugin
{
    ...
    public function plgVmConfirmedOrder($cart, $order)
    {
        ...
        if (!empty($designId)) {
            ...
            // Set cURL options
            curl_setopt_array($curl, [
                CURLOPT_URL => 'https://api.zakeke.com/v2/orders/'.$orderId, // Use the correct endpoint and orderCode (107 is a placeholder)
            ...
            // Handle errors
            if (curl_errno($curl)) {
                echo 'cURL Error: ' . curl_error($curl);  // Display error if cURL fails
            } else {
                $decodedResponse = json_decode($response, true); // Decode the JSON response into an associative array
                print_r($decodedResponse);  // Print the order details (you can process it further here)
            }
