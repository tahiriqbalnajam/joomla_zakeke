...
class plgVmCalculationAvalara extends vmCalculationPlugin {
	...
	public function getVmPluginCreateTableSQL() {
 		...
 		return "CREATE TABLE IF NOT EXISTS `" . $this->_tablename . "` (
 			...
 			) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Table for avalara' AUTO_INCREMENT=1 ;";
	...
	public function plgVmGetPluginInternalDataCalc(&$calcData){
		...
		if(!is_array($calcData->avatax_virtuemart_country_id)){
			//Suppress Warning
			$calcData->avatax_virtuemart_country_id = vmJsApi::safe_json_decode($calcData->avatax_virtuemart_country_id,true);
		...
		if(!empty($vmadd)){
			...
			if(empty($vmadd['virtuemart_country_id'])){
				...
				if(!is_array($calc['avatax_virtuemart_country_id'])){
					//Suppress Warning
					$calc['avatax_virtuemart_country_id'] = vmJsApi::safe_json_decode($calc['avatax_virtuemart_country_id'],true);
				}
		...
		try
		{
			...
			if ( $_taxResult[$shipmentMethodID]->getResultCode() == SeverityLevel::$Success){
				...
				if (!empty($virtuemart_order_id)) {
					...
					try {
						if ($db->execute() === false) {
							vmError('Avatax executeRequest error storing returned DocId: ');
						}
