...
/**
 * @version $Id: eway.php 11006 2024-05-08 10:18:01Z Milbo $
 * @package    VirtueMart
...
class plgVmpaymentEway extends vmPSPlugin {
	...
	private function createTokenCustomerIDTable($tablesFields = 0) {
	    ...
	    ) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Eway TokenCustomerID Table' AUTO_INCREMENT=1 ;";
...
// TODO GetTokenCustomerID
	...
	private function getEwaySelected($method, $cart) {
		...
		$maskedCardSelected = vmJsApi::safe_json_decode($maskedCardSelectedJson, false);
...
*/
	...
	static function getMaskedCardsFromSession() {
		...
		$maskedCards = vmJsApi::safe_json_decode($maskedCardsJson, false);
		return $maskedCards;
	}
	...
	function deleteCardConfirm($userId, $cardToDelete) {
		...
		$cardToDeleteUncrypted = vmJsApi::safe_json_decode($cardToDeleteUncrypted, false);
	...
	public function plgVmOnEwayDeleteConfirmCreditCard($element, $userId, $cardToDelete, &$html) {
		...
		$vendorId = 1;
		if ($this->getPluginMethods($vendorId, true, $userId) === 0) {
			return false;
		}
	...
	public function plgVmOnEwayDeleteCreditCard($element, $userId, $cardToDelete, &$maskedCards, &$msg) {
		...
		$cardToDelete = vmJsApi::safe_json_decode($cardToDelete, false);
	...
	public function plgVmOnEwayUpdateCreditCard($element, $userId, $cardToUpdate, &$html) {
		...
		$cardToUpdate = vmJsApi::safe_json_decode($cardToUpdate, false);
