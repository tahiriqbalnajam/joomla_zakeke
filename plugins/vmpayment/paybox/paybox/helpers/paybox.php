		...
		foreach ($payments as $payment) {
			...
			if ($first) {
				...
				if (!empty($payment->paybox_fullresponse)) {
					$paybox_data = vmJsApi::safe_json_decode($payment->paybox_fullresponse, false);
...
<CountryCode>'.$country_num_code.'</CountryCode>
	...
	function checkConditions ($cart, $method, $cart_prices) {
		...
		if(empty($method->min_amount)) $method->min_amount = 0;
		if(empty($method->max_amount)) $method->max_amount = 0;
